...
import regex as re
import html
# from transformer_decoder_ori import GPT2LMHeadModel_with_location
import random
import os
import sys
import tempfile
...
def tokenize_clip(caption):
    ...
    words=re.findall(pat, caption)
    return words


def noise_injection(x, variance=0.0001,device = None, modality_offset=None, uniform_noise=False, dont_norm=False):
    if variance == 0.0:
        return x
    ...
    if uniform_noise:
        ...
        x = x + (torch.randn(x.shape, device=device) * std)  # todo by some conventions multivraiance noise should 
...
class ClipCocoDataset(Dataset):
    ...
    def __init__(self, data_path: str,  prefix_length: int, gpt2_type: str = "gpt2",
        ...
        self.max_seq_len = 20
        with open('results_v3.json', 'r') as f:
...
def train_lstm(dataset: ClipCocoDataset, args,
...
def main():
    ...
    train_lstm(dataset, args, output_dir=args.out_dir, output_prefix=args.prefix)
